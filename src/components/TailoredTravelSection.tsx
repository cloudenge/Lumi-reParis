...
export const TailoredTravelSection: React.FC<TailoredTravelSectionProps> = ({ 
  onServiceSelect,
  onPartnerClick 
}) => {
  return (
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-start mb-24">
          ...
          >
            ...
            <div className="relative w-full h-40 mb-6 rounded-2xl bg-black/40 border border-white/15 p-4 flex items-center justify-center backdrop-blur-xl">
              {/* Grid pattern background */}
              <div className="absolute inset-0 opacity-10 bg-[radial-gradient(#fff_1px,transparent_1px)] [background-size:16px_16px]" />
              
              <svg className="w-full h-28 text-sky-400 drop-shadow-[0_0_12px_rgba(2,132,199,0.5)] overflow-visible" viewBox="0 0 350 100" fill="none" stroke="currentColor">
            ...
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-normal leading-tight text-white tracking-tight drop-shadow-sm">
          ...
          >
            ...
            <div className="p-5 rounded-2xl bg-black/40 border border-white/15 backdrop-blur-xl hover:border-white/30 transition-all shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-20">
          ...
          <motion.div 
            ...
            whileHover={{ scale: 1.03, y: -8, boxShadow: "0 30px 60px -15px rgba(2, 132, 199, 0.4)" }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ type: "spring", stiffness: 90, damping: 20, delay: 0.1 }}
            onClick={() => onServiceSelect?.('On-Demand Private Jet')}
            className="group relative h-[420px] rounded-3xl overflow-hidden border border-white/15 bg-black/40 backdrop-blur-2xl shadow-2xl flex flex-col justify-end p-6 transition-all duration-300 cursor-pointer hover:border-white/30"
          >
            <img 
            ...
            <div className="relative z-10">
              <h3 className="text-2xl sm:text-3xl font-normal font-headline-italic text-white mb-1 group-hover:text-sky-300 transition-colors drop-shadow-sm">
                On-Demand
              </h3>
          ...
          <motion.div 
            ...
            className="group relative h-[420px] rounded-3xl border border-white/15 bg-black/40 backdrop-blur-2xl shadow-2xl flex flex-col justify-between p-7 hover:border-white/30 transition-all duration-300 overflow-hidden cursor-pointer"
          >
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 border border-white/15 text-xs text-white/90 mb-8">
                <Sliders className="w-3.5 h-3.5 text-sky-400" />
                <span>Tailored Charter</span>
              </div>

              <h3 className="text-3xl sm:text-4xl font-normal font-headline-italic text-white mb-3 tracking-tight drop-shadow-sm">
                Flexible <br />
            ...
            <div>
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  onServiceSelect?.('Flexible Charter Solutions');
                }}
                className="group/btn inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/10 hover:bg-white/20 border border-white/20 text-xs font-medium text-white transition-all cursor-pointer active:scale-95"
              >
          ...
          >
            <img 
              ...
              alt="Executives in airport lounge" 
              className="absolute inset-0 w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-700 opacity-75 group-hover:opacity-90"
              loading="lazy"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/40 to-transparent" />
            ...
            <div className="relative z-10">
              <h3 className="text-2xl sm:text-3xl font-normal font-headline-italic text-white mb-1 group-hover:text-amber-300 transition-colors drop-shadow-sm">
                Shared Flights
              </h3>
              <p className="text-xs sm:text-sm text-white/80 leading-relaxed">
        ...
        <motion.div 
          ...
          className="pt-8 border-t border-white/10 flex flex-wrap items-center justify-center sm:justify-between gap-6 sm:gap-8 text-white/50 font-mono text-xs sm:text-sm tracking-wider uppercase"
        >
          {['Southwest', 'British Airways', 'AIRBUS', 'AIRFRANCE', 'RYANAIR'].map((partner) => (
            <button
              key={partner}
              onClick={() => onPartnerClick?.(partner)}
              className="hover:text-white transition-colors cursor-pointer focus:outline-none focus:text-white py-1 px-2 rounded-lg hover:bg-white/5"
            >
              {partner === 'British Airways' ? (
                <span className="font-serif lowercase italic text-base">{partner}</span>
              ) : partner === 'AIRBUS' ? (
                <span className="border border-white/20 px-2 py-0.5 rounded text-[11px] font-bold">{partner}</span>
              ) : partner === 'RYANAIR' ? (
                <span className="font-sans font-bold text-amber-400/90">{partner}</span>
              ) : (
                <span>{partner}</span>
              )}
            </button>
