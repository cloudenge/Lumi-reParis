...
export const ExecutiveTravelSection: React.FC<ExecutiveTravelSectionProps> = ({ 
  onReserveClick,
  onCardClick 
}) => {
  return (
    <section id="executive" className="w-full bg-transparent text-white py-16 sm:py-24 px-4 sm:px-6 lg:px-8 border-t border-white/10 relative overflow-hidden">
      ...
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-sky-500/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          ...
          <motion.div 
            ...
            whileHover={{ scale: 1.03, y: -8, boxShadow: "0 25px 50px -12px rgba(2, 132, 199, 0.3)" }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ type: "spring", stiffness: 100, damping: 20, delay: 0.1 }}
            onClick={() => onCardClick?.('Instant Reservations')}
            className="bg-black/40 backdrop-blur-2xl rounded-3xl p-6 sm:p-7 shadow-2xl border border-white/15 flex flex-col justify-between transition-all duration-300 group cursor-pointer hover:border-white/30"
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <Zap className="w-4 h-4 text-white/50 group-hover:text-sky-400 transition-colors" />
              </div>
          ...
          <motion.div 
            initial={{ opacity: 0, y: 60, scale: 0.95 }}
            whileInView={{ opacity: 1, y: 0, scale: 1 }}
            whileHover={{ scale: 1.03, y: -8, boxShadow: "0 25px 50px -12px rgba(245, 158, 11, 0.3)" }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ type: "spring", stiffness: 100, damping: 20, delay: 0.2 }}
            onClick={() => onCardClick?.('Global Fleet & Fly Anywhere')}
            className="bg-black/40 backdrop-blur-2xl rounded-3xl p-6 sm:p-7 shadow-2xl border border-white/15 flex flex-col justify-between transition-all duration-300 group cursor-pointer hover:border-white/30"
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <Globe className="w-4 h-4 text-white/50 group-hover:text-amber-400 transition-colors" />
              </div>
            ...
            <div className="w-full h-44 sm:h-48 rounded-2xl overflow-hidden bg-black/40 border border-white/10">
              <img 
                ...
                alt="Gulfstream jet flying above clouds"
                className="w-full h-full object-cover object-center group-hover:scale-105 transition-transform duration-500 opacity-90 group-hover:opacity-100"
                loading="lazy"
              />
            </div>
          ...
          <motion.div 
            initial={{ opacity: 0, y: 60, scale: 0.95 }}
            whileInView={{ opacity: 1, y: 0, scale: 1 }}
            whileHover={{ scale: 1.03, y: -8, boxShadow: "0 25px 50px -12px rgba(16, 185, 129, 0.3)" }}
            viewport={{ once: true, margin: "-50px" }}
            transition={{ type: "spring", stiffness: 100, damping: 20, delay: 0.3 }}
            onClick={() => onCardClick?.('Uncompromised Safety Standards')}
            className="bg-black/40 backdrop-blur-2xl rounded-3xl p-6 sm:p-7 shadow-2xl border border-white/15 flex flex-col justify-between transition-all duration-300 group cursor-pointer hover:border-white/30"
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <ShieldCheck className="w-4 h-4 text-white/50 group-hover:text-emerald-400 transition-colors" />
              </div>
        ...
        >
          <p className="text-sm sm:text-base text-white/80 font-normal leading-relaxed mb-8">
          ...
          <button
            ...
            className="px-8 py-3.5 rounded-full bg-[#0284c7] hover:bg-[#0369a1] text-white font-medium text-sm transition-all duration-200 shadow-xl shadow-sky-500/20 active:scale-95 cursor-pointer flex items-center gap-2 group border border-sky-400/30"
          >
        ...
        <motion.div 
          ...
          onClick={() => onCardClick?.('Premier Experience & Personalized Concierge')}
          className="w-full h-[320px] sm:h-[450px] rounded-3xl overflow-hidden relative shadow-2xl border border-white/15 group bg-black/40 backdrop-blur-2xl cursor-pointer hover:border-white/30 transition-all duration-300"
        >
          <img 
            ...
            className="w-full h-full object-cover object-center group-hover:scale-103 transition-transform duration-700 opacity-85 group-hover:opacity-100"
            loading="lazy"
          />
          ...
          <div className="absolute bottom-6 left-6 right-6 text-white sm:max-w-md">
            ...
            <h4 className="text-xl sm:text-2xl font-normal font-headline-italic text-white drop-shadow-sm">
